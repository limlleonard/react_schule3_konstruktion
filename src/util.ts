...
const FE1:Punkt=[width/2, height/2]; // F1 für Ellipse
const FE2:Punkt=[width / 2 - width / 5, height / 2];
const FH1:Punkt=[width * 3 / 4, height / 2];
const FH2:Punkt=[width / 2, height / 2];
const FP:Punkt=[width / 2, height / 2];
...
// Es ist in Wahrheit kein Winkel, aber wird es so genannt für die Einheitlichkeit
for (let x = 0; x <= width/2; x += width/8) {
    angles[2].push(x);
    if (x!==0) angles[2].push(-x);
}

let points:Punkt[]=[]; // Finale Punkte
export const resetPoints = () => {points=[]};

export const vorbereitungen:string[][]=[
    [
...
export const anweisungen:string[][]=[
...
export let sumSteps:number[]=[];
for (let i=0; i<3; i++) {
    sumSteps.push(vorbereitungen[i].length + angles[i].length*anweisungen[i].length);
}

function mlPoint(ctx:CanvasRenderingContext2D, p1:Punkt, color:string='white', r=Rpoint){ // draw point
    ctx.beginPath();
...
function mlCircle(ctx:CanvasRenderingContext2D, p1:Punkt, r:number, color='white'){
    ctx.beginPath();
    ctx.arc(...p1, r, 0, Math.PI*2);
    ctx.strokeStyle = color;
    ctx.stroke();
}
function mlLine(ctx:CanvasRenderingContext2D, p1:Punkt,p2:Punkt,color='white'){
    ctx.beginPath();
    ctx.moveTo(...p1);
    ctx.lineTo(...p2);
    ctx.strokeStyle=color;
...
function mlText(ctx:CanvasRenderingContext2D, p1:Punkt, text:string) {
    ctx.font = '20px Arial';
    ctx.textAlign = 'left';
    ctx.fillStyle = 'white';
...
function mlStrecke2Gerade(p1:Punkt, p2:Punkt, w:number, h:number) {
    const [x1, y1] = p1;
    const [x2, y2] = p2;
    const dx = x2 - x1;
    const dy = y2 - y1;
    if (dx === 0 && dy === 0) {
        throw new Error("Die zwei Punkte muss unterschiedlich sein");
    }
    const intersections:Punkt[] = [];
...
function schnittKreise(p1:Punkt, p2:Punkt, distance:number):null|[Punkt,Punkt] {
    ...
    const intersection1:Punkt = [x0 + rx, y0 + ry];
    const intersection2:Punkt = [x0 - rx, y0 - ry];
    return [intersection1, intersection2];
}
// Input: zwei Punkte einer strecke. Output: zwei Schnittunkts von der Mittelsenkrecht Gerade und Grenze der Canvas
function mittelsenkrecht(p1:Punkt, p2:Punkt):[Punkt,Punkt]{
    const [x1,y1]=p1;
    const [x2,y2]=p2;
...
function schnittGerade(p1:Punkt,p2:Punkt,p3:Punkt,p4:Punkt):Punkt{
...
function abstand(p1:Punkt,p2:Punkt):number {
    return ( (p1[0]-p2[0])**2+(p1[1]-p2[1])**2 )**0.5
}

function zeichnenVorbereiten(ctx:CanvasRenderingContext2D, mode:number, nrVorbereitung:number):void{
    if (mode==0) {
        if (nrVorbereitung>-1) {
        ...
        throw new Error("Es gibt nur 3 Modi")
    }
}

function zeichnenEllipse(ctx:CanvasRenderingContext2D, nrAnweisung:number, angle:number, F1:Punkt, F2:Punkt) {
    // moving point
    ...
    if (nrAnweisung!=anweisungen[0].length-1) {
        ...
        // schnitt zwei kreise
        if (nrAnweisung>4) {
            // const [pSchnittCircle1, pSchnittCircle2]: [Punkt, Punkt] = schnittKreise([x, y], F2, dist) ?? [[0, 0], [0, 0]];
            // const result = schnittKreise([x, y], F2, dist);
            // result?.[0] && result?.[1] && doSomething(result[0], result[1]);
            const [pSchnittCircle1, pSchnittCircle2] = schnittKreise([x, y], F2, dist) as [Punkt, Punkt]; // xxts

            mlPoint(ctx, pSchnittCircle1, 'blue');
            mlPoint(ctx, pSchnittCircle2, 'blue');}
            // senkrecht
            if (nrAnweisung>5) {
                const [p1,p2]:[Punkt,Punkt]=mittelsenkrecht([x,y], F2);
...
function zeichnenHyperbel(ctx:CanvasRenderingContext2D, nrAnweisung:number, angle:number, F1:Punkt, F2:Punkt) {
    // moving point
    ...
    if (nrAnweisung!=anweisungen[1].length-1) {
        ...
        // schnitt zwei kreise
        if (nrAnweisung>4) {
        const [pSchnittCircle1, pSchnittCircle2]=schnittKreise([x,y],F2, dist) as [Punkt,Punkt]
        mlPoint(ctx, pSchnittCircle1, 'blue');
        mlPoint(ctx, pSchnittCircle2, 'blue');}
        // senkrecht
        if (nrAnweisung>5) {
...
function zeichnenParabel(ctx:CanvasRenderingContext2D, nrAnweisung:number, angle:number, F:Punkt) {
    // moving point
    let x:number=width/2+angle;
    let y:number=yParabel;
    let p1:Punkt;
    let p2:Punkt;
    let p3:Punkt;
    let p4:Punkt;
    const dist:number=abstand([x,y],F)
    if (nrAnweisung!=anweisungen[2].length-1) {
        ...
        // schnitt zwei kreise
        if (nrAnweisung>4) {
        const [pSchnittCircle1, pSchnittCircle2]=schnittKreise([x+rParabel,y],[x-rParabel, y], rParabel*2) as [Punkt,Punkt];
        mlPoint(ctx, pSchnittCircle1, 'blue');
        mlPoint(ctx, pSchnittCircle2, 'blue');}
        // senkrecht
        if (nrAnweisung>5) {
        ...
        if (nrAnweisung>9) {
            const [pSchnittCircle3, pSchnittCircle4]=schnittKreise([x,y],F, dist) as [Punkt,Punkt];
...
export function zeichnen(ctx:CanvasRenderingContext2D, mode:number, step:number) {
